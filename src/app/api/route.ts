...
export async function POST(request: NextRequest): Promise<NextResponse> {
    ...
    // This indicates a LinkedIn webscraper response to which webScraper.ts will take over the POST request
    if (data[data.length - 1].content.startsWith('Give me some connections relating to these keywords:')) {
        const keywords: string = data.content.split(':')[1]
        const webScraperResponse = await webScraper.getTop10Connections(keywords);
        // Streams LinkedIn web scraper response
        const stream = new ReadableStream({
            async start(controller) {
                try {
                    controller.enqueue(webScraperResponse);
                } catch (err) {
